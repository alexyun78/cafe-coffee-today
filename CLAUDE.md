...
2. **노션에 후보글 (복수)** — Claude 가 **같은 노션 페이지 안에** `divider` + `## ✍️ 후보 A — <제목>` 형태로 후보를 **여러 개** append 한다. 톤(담백/유머/서정), 문체, 구성이 서로 다른 안을 올려 사용자가 고를 수 있게 한다. **이 단계에서 사이트 글(`story/*.html`, `index.json`)에는 손대지 않는다.**
3. **최종 글 확정** — 사용자가 고른 안을 다듬어 `## ✍️ 최종본 — <제목>` 으로 노션에 남긴다. 노션이 원고의 기록처다.

> 📷 **노션 원고에도 사진을 그대로 넣는다 (사이트 발행 형식과 1:1).** 텍스트만 올리지 말 것.
> 사진 위치는 사이트 `<figure>` 자리 그대로, 캡션은 `figcaption` 그대로 노션 `image` 블록의 caption 으로.
> - 방식: `{"type":"image","image":{"type":"external","external":{"url":"https://92cafe.co.kr/static/img/story/<no>-<i>.jpg"},"caption":[...]}}`
> - 그래서 **사진 파일은 2단계 전에 먼저 커밋·푸시**해 공개 URL 을 만들어 둔다 (이미지만 올리는 건 글 발행이 아니므로 절차 위반 아님).
> - 92cafe.co.kr 은 User-Agent 가 아예 없는 요청만 403 — 노션 이미지 프록시는 정상 로드됨(2026-08-08 확인).
> - 원고를 고쳐 다시 올릴 때는 기존 `✍️` 섹션 블록을 지우고 통째로 재 append 하는 편이 안전하다(노션은 중간 삽입이 까다로움). 사용자가 쓴 초안 블록은 절대 건드리지 말 것.
> - **역방향(노션 수정 → 사이트 반영)**: 노션 블록을 그대로 `story/*.html` 본문으로 다시 생성하되, 노션에 없는 정보는 기존 HTML 값을 보존한다 — 특히 **`img alt`**(노션 image 블록엔 alt 개념이 없어 캡션으로 덮어써 버리기 쉽다), `class="lead"`, `figure.doc`. 반영 전 기존 본문과 텍스트 diff 를 떠서 사용자가 바꾼 지점만 바뀌었는지 확인할 것.
...
- 홈 네비에 **로그인 버튼이 항상 있다**(비로그인=`로그인`, 로그인=`<호칭> 기록`). 가입은 여전히 QR 카드로만 되지만,
...
- **오늘의 커피 → 원두 카드 연결** (2026-09-13): 공개 `/api/coffee` 가 항목마다 `원두카드`(카드 id 또는 null)를 붙인다.
  `coffees.green_bean_id`(비었으면 같은 이름의 생두) → `index.json` 의 `green_bean_id` 로 찾고, 시크릿 미션과 블랜드 카드는 null.
  홈 NOW SERVING 카드 전체와 곧 만날 커피 모달의 "원두 이야기 보기" 가 `/beans/<id>` 로 간다. **카드에 `green_bean_id` 가 없으면 연결도 안 된다.**
...
## 한글 문장 작성 규칙 (모든 사용자 노출 글 공통)

인사이트, 92스토리, 원두 카드, 노션 초안 등 **사람이 읽는 모든 한국어 본문**에 적용.

- **나열은 가운뎃점(`·`)이 아니라 쉼표로 쓴다.** `칼·마이클·리카르도·피터` ❌ → `칼, 마이클, 리카르도, 피터` ✅.
  두 항목이면 `A와 B` 도 좋다 (`바리스타·브루어스컵` ❌ → `바리스타와 브루어스컵` ✅).
  가운뎃점 나열은 AI가 쓴 티가 확 나는 대표적인 문체다. (2026-08-09 사용자 지적)
  - 예외: **본문이 아닌 UI 라벨/메타 줄**은 기존 그대로 둔다 — 관리자 목록의 `로스터리 · 🫘 로스팅일 · ☕ 제공일`,
    탭 이름, 표 헤더 같은 곳. 문장 안에서 나열할 때만 쉼표로 바꾼다.
