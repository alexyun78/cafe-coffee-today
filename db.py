...
_GB_EXTRA_COLUMNS = (
    ...
    ("bean_type", "TEXT NOT NULL DEFAULT '싱글'"),
    # 품절 플래그. status(활성/단종)와 별개 — 품절은 목록에 남되 지금 안 파는 상태.
    ("sold_out", "INTEGER NOT NULL DEFAULT 0"),
)
...
def _parse_ymd(d) -> tuple:
    """'YYYY-MM-DD', 'YYYY. M. D', 'YYYY.MM.DD' 등 다양한 형식을 (연,월,일) 튜플로."""
    m = re.search(r"(\d{4})\D+(\d{1,2})\D+(\d{1,2})", d or "")
    if m:
...
def _norm_ymd(d):
    """날짜를 'YYYY-MM-DD'로 정규화 (파싱 실패 시 원본 반환)."""
    y, mo, da = _parse_ymd(d)
    return f"{y:04d}-{mo:02d}-{da:02d}" if y else d
...
def list_roasting_logs(green_bean_id: Optional[int] = None, limit: int = 1000) -> list:
    ...
    # 마이그레이션 잔재로 날짜 형식이 섞여 있어도 최근 날짜순이 정확하도록
